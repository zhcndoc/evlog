...
- 使用 `data` 存放客户端需要据此进行分支处理的值，例如订单 ID、重试截止时间或无效字段列表。`why`、`fix` 和 `link` 用于存放供人阅读的字符串
- 负载会合并到响应正文的 `data` 对象中，与 `code`、`why`、`fix` 和 `link` 并列；发生键冲突时，以这四个字段为准。在客户端通过 `parseError(error).data` 读取
- 字段名与 h3 的 `createError({ data })` 相同，并且 evlog 的 Nitro 错误处理器也会为 h3 错误转发该字段
...
`EvlogError.toJSON()`，由 Next.js 处理器原样返回：
...
Nitro、Nuxt 和 SvelteKit 处理器会返回 Nitro 格式的正文，并在相同的 `message`、`status` 和 `data` 外层附带 `url`、`statusCode`、`statusText` 以及 `error: true`。所有序列化器在客户端读取的部分上保持一致：`why`、`fix` 和 `link` 位于 `data` 下，而不是顶层；`internal` 和 `stack` 均不会被包含。在客户端，`parseError()` 会将这两种格式展平。
