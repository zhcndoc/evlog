...
本指南介绍你在使用 evlog 时最常用的核心 API。更喜欢通过运行一个完整的服务器来学习？请按照 [Node.js 中的结构化日志记录](/use-cases/structured-logging-nodejs)进行操作，然后回到这里查看 API 概览。

::callout{icon="i-lucide-sparkles" color="info"}
在 Nuxt 中，evlog 会**自动导入** `useLogger`、`log` 和 `parseError`。`createError` 从 `evlog` 中导入：自动导入的那个是 Nuxt 自带的，并且它会丢弃 `why`、`fix` 和 `link`。
::
