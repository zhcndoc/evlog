...
The simplest way to use evlog. Send structured logs anywhere, anytime, without worrying about them:
...
Two calling styles:
- **Tagged**: `log.info('tag', 'message')`, suitable for quick, readable console output
- **Structured**: `log.info({ key: value })`, suitable for rich events passed through the drain pipeline

::callout{icon="i-lucide-arrow-right" color="neutral"}
See the complete [Simple Logging](/learn/simple-logging) guide to learn all patterns and drain integration.
::
...
::callout{icon="i-lucide-arrow-right" color="neutral"}
查看 [客户端日志](/use-cases/client-logging) 了解传输配置、身份上下文和浏览器 drain 设置。
::

## 下一步

- [`evlog map`](/cli/map)：查看你刚刚解锁了什么——评估覆盖率并找到仍然处于暗区的入口点（`npx @evlog/cli map`）
- [日志概览](/learn/overview)：了解全部三种日志模式
- [宽事件](/learn/wide-events)：学习如何设计有效的宽事件
- [类型化字段](/learn/typed-fields)：为你的宽事件添加编译期类型安全
- [结构化错误](/learn/structured-errors)：掌握使用 evlog 进行错误处理
- [最佳实践](/reference/best-practices)：安全指南和生产环境提示
