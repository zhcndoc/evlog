---
title: evlog — 深入挖掘日志不是可观测性。这只是希望。
...
#body
  ...
  #description
  作为宽事件之上的一层薄封装，提供一流的“谁做了什么”追踪。一个增强器、一个排水包装器、一个辅助函数。防篡改哈希链、被拒绝的操作、支持脱敏的差异，以及用于安全重试的幂等键——全部来自主入口点，没有并行管道。
  :::
