...
#body
  ...
  #description
  大多数团队第一次发现某个处理器根本没记录日志，往往是在事故进行中。把它想象成 Lighthouse，但用于可观测性：为你的应用在出问题时能提供的上下文打分，并给出准确的修复清单——在真正重要之前。
  :::
  ...
  #description
  一流的“谁在何时做了什么”轨迹，作为宽事件之上的一层薄封装。一个 enricher、一个 drain 包装器、一个 helper。防篡改哈希链、被拒绝的操作、可感知 redaction 的差异、用于安全重试的幂等键，以及用于重构安全告警的类型化操作目录——全部来自主入口点，没有并行管道。
  :::
