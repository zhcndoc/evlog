...
宽事件包含请求触及的每个字段，这正是它可查询的原因，也是意外泄露的密钥永久存在的原因。本页介绍哪些内容应排除、哪些内容应脱敏，以及生产环境中的成本。

## 不应记录的内容

使宽事件值得查询的累积特性，也意味着一旦某个密钥到达 `log.set`，它就会进入你的排水系统。**永远不要记录：**
...
这会在每个宽事件中屏蔽信用卡（`****1111`）、电子邮件（`a***@***.com`）、IP、电话号码、JWT、Bearer 令牌和 IBAN，并且会在输出到控制台以及发送到任何排水服务之前完成。有关完整的配置参考，请参阅 [Auto-Redaction](/learn/redaction)。
...
日志量决定你的成本，而一个无人查看的成功请求与一个失败请求的成本相同。保留所有错误，减少其余日志：
...
使用 `$production` 覆盖在开发环境保留完整日志，而在生产环境启用采样。参见 [Nuxt 框架指南](/integrate/frameworks/nuxt#sampling)。
::

## 下一步

- [Auto-Redaction](/learn/redaction) - 内置的 PII 保护与智能掩码
- [Wide Events](/learn/wide-events) - 设计有效的宽事件
