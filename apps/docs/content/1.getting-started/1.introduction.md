...
- **Zero transitive dependencies** and ~6 kB gzip — nothing to audit, nothing that breaks on the next Node LTS. Benchmarked at [~3 µs/request](/core-concepts/performance), 7.7x faster than pino in the wide event pattern (1 event vs 4 log lines) and competitive on every other path.
