...
文档：https://www.evlog.dev/adapters/self-hosted/nuxthub
...
evlog 发出的每个宽事件都会作为一行存储在 `evlog_events` 表中。该 drain 插件同时处理单个事件和批量事件（与 [pipeline](/adapters/building-blocks/pipeline) 一起使用时）。
...
- [适配器](/adapters/overview) - 将日志发送到外部服务，同时使用 NuxtHub 存储
- [Pipeline](/adapters/building-blocks/pipeline) - 批处理事件以提升数据库性能
