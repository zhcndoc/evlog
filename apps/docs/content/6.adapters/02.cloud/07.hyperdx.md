...
适配器文档：https://www.evlog.dev/adapters/cloud/hyperdx
...
在底层，`createHyperDXDrain()` 会将你的 HyperDX 设置映射到共享的 [OTLP 适配器](/adapters/cloud/otlp)，并调用 `sendBatchToOTLP()`：
...
- [OTLP Adapter](/adapters/cloud/otlp) - 通过 OpenTelemetry 协议向任何 OTLP 后端发送日志
- [PostHog Adapter](/adapters/cloud/posthog) - 通过 OTLP 将日志发送到 PostHog Logs
- [Custom Adapters](/adapters/building-blocks/custom) - 构建你自己的适配器
- [Best Practices](/core-concepts/best-practices) - 安全和生产环境提示
