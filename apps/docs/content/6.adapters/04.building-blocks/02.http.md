...
为我的浏览器日志设置 HTTP 传输，使其发送到我的服务器。

- 安装 evlog：pnpm add evlog
- 从 'evlog/http' 导入 createHttpLogDrain（不是 evlog/browser——它已弃用）
- 使用 { endpoint: 'https://logs.example.com/v1/ingest' } 和可选的管道（{ batch: { size, intervalMs } }）创建一个 drain
- 在客户端将 drain 传递给 initLogger({ drain })
- drain 会批处理事件，并在可见性变化时使用 fetch keepalive + sendBeacon
- 在服务器端，接受带有 DrainContext[] 请求体的 POST 请求，并将它们转发到我的 drain 管道

文档：https://www.evlog.dev/adapters/building-blocks/http
管道：https://www.evlog.dev/adapters/building-blocks/pipeline
...
app.post('/v1/ingest', express.json(), (req, res) => {
  for (const entry of req.body) {
    console.log('[浏览器]', JSON.stringify(entry))
  }
  res.sendStatus(204)
})
```
...
const pipeline = createDrainPipeline<DrainContext>({
  ...
  maxBufferSize: 500,
  onDropped: (events) => {
    console.warn(`丢弃了 ${events.length} 个客户端事件`)
  },
})
...
- [管道](/adapters/building-blocks/pipeline) - 批处理、重试和缓冲区溢出处理
- [自定义适配器](/adapters/building-blocks/custom) - 构建您自己的 drain 函数
