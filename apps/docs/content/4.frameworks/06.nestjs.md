...
使用 `req.log.fork(label, fn)`（或同一请求中来自 `useLogger()` 的日志记录器）创建子广泛事件。请参阅 [广泛事件 — 发射后](/logging/wide-events#after-emit-sealing-and-background-work)。
...
在服务器关闭时调用 `drain.flush()` 以确保发送所有缓冲的事件。有关所有选项，请参阅 [Pipeline 文档](/adapters/building-blocks/pipeline)。
::
