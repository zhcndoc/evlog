...
Hono 尚未附加 **`log.fork()`**（仅通过 `c.get('log')` 访问日志记录器）。如果您在响应之后调度异步工作，发出后的 **`[evlog]` 警告** 仍然可以帮助您注意到过时的 `set()` 调用。请参阅 [广泛事件 — 发出后](/logging/wide-events#after-emit-sealing-and-background-work)。
...
在服务器关闭时调用 `drain.flush()`，以确保发送所有缓冲的事件。有关所有选项，请参阅 [管道文档](/adapters/building-blocks/pipeline)。
::
...
有关批处理、重试、sendBeacon 回退和身份验证选项，请参阅完整的 [HTTP drain](/adapters/building-blocks/http) 适配器文档。
::
