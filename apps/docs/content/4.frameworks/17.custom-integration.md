...
- Import { createMiddlewareLogger, extractSafeHeaders, createLoggerStorage } from 'evlog/toolkit' (NOT 'evlog/shared')
- Extract method, path, requestId, and headers from the framework's request object (use extractSafeHeaders or extractSafeNodeHeaders)
- Call createMiddlewareLogger({ method, path, requestId, headers, ...userOptions }) at request start
- If result.skipped is true (route filtered out), skip to next middleware
- Store result.logger in the framework's idiomatic context (req.log, c.set('log'), etc.) and via createLoggerStorage for useLogger()
- On response: call finish({ status }) on success or finish({ error }) on failure to trigger emit + enrich + drain
- Expose drain, enrich, keep, include, exclude, and routes options to my users

Docs: https://www.evlog.dev/frameworks/custom-integration
Toolkit: https://www.evlog.dev/frameworks/overview
