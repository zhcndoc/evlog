...
使用 `locals.log.fork(label, fn)` 创建子宽事件。请参阅 [宽事件 — 发出后](/logging/wide-events#after-emit-sealing-and-background-work)。
...
在服务器关闭时调用 `drain.flush()`，以确保所有缓冲的事件都被发送。请参阅 [管道文档](/adapters/building-blocks/pipeline) 了解所有选项。
::
