...
在服务器关闭时调用 `drain.flush()`，以确保所有缓冲的事件都被发送。请参阅 [Pipeline 文档](/adapters/building-blocks/pipeline) 了解所有选项。
::
...
请参阅完整的 [HTTP drain](/adapters/building-blocks/http) 适配器文档，了解批处理、重试、sendBeacon 回退和身份验证选项。
::
