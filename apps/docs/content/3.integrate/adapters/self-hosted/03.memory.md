...
Memory 适配器会将宽事件存储在模块级环形缓冲区中。与 [文件系统适配器](/integrate/adapters/self-hosted/fs) 不同，它**没有运行时依赖**，并且可以在任何地方运行——包括 Cloudflare Workers（workerd）、Deno Deploy，以及其他不提供 Node `fs` 模块的边缘运行时。
