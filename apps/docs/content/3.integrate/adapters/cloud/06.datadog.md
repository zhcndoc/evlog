...
如需改用基于 OpenTelemetry 的摄入方式，请参阅 [OTLP 适配器](/integrate/adapters/cloud/otlp)。
...
- **`service`**、**`status`**（Datadog 严重级别 — 决定实时追踪颜色）、**`ddsource`**: `evlog`、**`ddtags`**: `env:…` 和可选 `version:…`
...
- [OTLP Adapter](/integrate/adapters/cloud/otlp) — 通过 OpenTelemetry 发送日志（适用于 Datadog Agent / OTLP 端点）
- [Custom Adapters](/extend/custom-drains) — 构建你自己的目标端
