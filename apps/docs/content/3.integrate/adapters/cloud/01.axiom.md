...
在 Axiom 的仪表板中，这个凭证称为 **API token**（`xaat-...`）。evlog 将配置字段命名为 `apiKey`，以便在各个适配器之间保持一致。旧的 `token` / `AXIOM_TOKEN` 仍然可用，直到下一个主要版本发布。
::
...
- [Best Practices](/reference/best-practices) - 安全和生产建议
