...
Hono 目前还不会附加 **`log.fork()`**（仅通过 `c.get('log')` 访问日志记录器）。如果你在响应后安排异步工作，发出后 **`[evlog]` 警告** 仍会帮助你发现过期的 `set()` 调用。参见 [广泛事件 — 发出后](/learn/wide-events#after-emit-sealing-and-background-work)。
