---
title: NestJS
description: NestJS 应用中的自动广泛事件、结构化错误、排水适配器、增强器和尾部抽样。
navigation:
  title: NestJS
...
使用 `req.log.fork(label, fn)`（或同一请求中来自 `useLogger()` 的日志记录器）创建子广泛事件。参见 [广泛事件 — 发送后](/learn/wide-events#after-emit-sealing-and-background-work)。
