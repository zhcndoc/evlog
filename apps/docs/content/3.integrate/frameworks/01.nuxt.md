...
| 选项 | 类型 | 默认值 | 描述 |
|--------|------|---------|-------------|
| `enabled` | `boolean` | `true` | 全局启用/禁用所有日志记录。当 `false` 时，所有操作都将变为无操作 |
...
| `pretty` | `boolean` | 开发环境中的 `true` | 使用树状格式进行美化输出 |
| `dev` | `'evlog' \| 'nitro' \| 'both' \| object` | 预格式化开发环境中的 `'evlog'` | 开发终端预设或 `{ frameworkOverlay, prettyError }` — 请参阅 [配置 — 开发终端输出](/reference/configuration#dev-terminal-output) |

::callout{icon="i-lucide-terminal" color="info"}
**开发终端预设：** `'evlog'`（默认）— 一个干净的信号，仅包含 evlog 堆栈。`'nitro'` — 广泛的事件上下文 + Nitro Youch 堆栈（evlog 仅打印 Why/Fix）。`'both'` — 完整的 evlog 块和 Nitro 覆盖层。使用 `pretty: false` 时，设置 `dev: { frameworkOverlay: false }` 以在记录 JSON 时抑制 Nitro。
::
