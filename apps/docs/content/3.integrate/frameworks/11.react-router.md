...
来自 `loggerContext` 的日志记录器支持 `fork` 用于子广泛事件。参见 [Wide events — After emit](/learn/wide-events#after-emit-sealing-and-background-work)。
...
- [Wide Events](/learn/wide-events): 设计具有上下文分层的完整事件
- [Adapters](/integrate/adapters/overview): 将日志发送到 Axiom、Sentry、PostHog 等
- [Sampling](/learn/sampling): 通过头部和尾部采样控制日志量
- [Structured Errors](/learn/structured-errors): 抛出带有 `why`、`fix` 和 `link` 字段的错误
