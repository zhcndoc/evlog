...
Nitro 使用 **`useLogger(event)`**（基于 event 的作用域），而不是 `AsyncLocalStorage`，所以这里 **`log.fork()` 暂不可用**。如果代码在广泛事件发出后仍调用 `set()`，则仍适用事后发出警告。请参见 [广泛事件 — 发出后](/learn/wide-events#after-emit-sealing-and-background-work)。
