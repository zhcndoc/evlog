---
title: 增强器概览
description: 使用增强器为广泛事件添加派生上下文，如用户代理、地理数据、请求大小和跟踪上下文。支持内置增强器和自定义增强器。
navigation:
  title: 概览
  icon: i-lucide-sparkles
links:
  - label: 内置增强器
    icon: i-lucide-puzzle
    to: /enrichers/built-in
    color: neutral
    variant: subtle
  - label: 自定义增强器
    icon: i-lucide-code
    to: /enrichers/custom
    color: neutral
    variant: subtle
---

Enrichers 在事件发出后、到达排放适配器之前，为广泛事件添加派生上下文。使用它们可以自动从请求头中提取有用信息，而不会使应用程序代码变得杂乱。

::enricher-chain
::

::prompt
---
description: 向我的应用添加 evlog 增强器
icon: i-lucide-sparkles
actions:
  - copy
  - cursor
  - windsurf
---

向我的应用添加 evlog 增强器，让广泛事件自动包含派生上下文。

- 确定我的框架，并遵循其 evlog 接线模式
- 从 'evlog/enrichers' 导入我需要的增强器（createUserAgentEnricher、createGeoEnricher、createRequestSizeEnricher、createTraceContextEnricher）
- 对于 Nuxt/Nitro：在服务器插件中将它们注册到 evlog:enrich 钩子上
- 对于 Hono/Express/Fastify/Elysia/NestJS：将它们作为 { enrichers: [...] } 传递给中间件
- 对于 Standalone：将它们传递给 initLogger({ enrichers: [...] })
- 增强器在 emit 之后、drain 之前运行——它们只添加字段，绝不会抛出异常
- 如果我希望增强器覆盖我手动设置的字段，传入 { overwrite: true }

文档：https://www.evlog.dev/enrichers/overview
内置：https://www.evlog.dev/enrichers/built-in

::

## Enrichers 的工作方式

Enrichers 钩入 `evlog:enrich` 事件，该事件在广泛事件发出后、`evlog:drain` 钩子之前触发。Enricher 接收事件和请求元数据，并可以修改事件以添加派生字段。

```
请求 → emit() → evlog:enrich → evlog:drain
                    ↑ enrichers      ↑ adapters
                    添加上下文      发送到服务
```

::code-group
```typescript [Nuxt / Nitro]
// server/plugins/evlog-enrich.ts
import { createUserAgentEnricher, createGeoEnricher } from 'evlog/enrichers'

export default defineNitroPlugin((nitroApp) => {
  const enrichers = [createUserAgentEnricher(), createGeoEnricher()]

  nitroApp.hooks.hook('evlog:enrich', (ctx) => {
    for (const enricher of enrichers) enricher(ctx)
  })
})
```
```typescript [Next.js]
// lib/evlog.ts
import { createEvlog } from 'evlog/next'
import { createUserAgentEnricher, createGeoEnricher } from 'evlog/enrichers'

const enrichers = [createUserAgentEnricher(), createGeoEnricher()]

export const { withEvlog, useLogger, log, createError } = createEvlog({
  service: 'my-app',
  enrich: (ctx) => {
    for (const enricher of enrichers) enricher(ctx)
  },
})
```
```typescript [Hono / Express / Fastify / Elysia / NestJS]
// 将 `enrichers` 传递给框架的 evlog 中间件/模块
import { createUserAgentEnricher, createGeoEnricher } from 'evlog/enrichers'

const enrichers = [createUserAgentEnricher(), createGeoEnricher()]

app.use(evlog({ enrichers })) // Hono / Express / Elysia
// await app.register(evlog, { enrichers }) // Fastify
// EvlogModule.forRoot({ enrichers }) // NestJS
```
```typescript [Standalone]
// index.ts
import { initLogger } from 'evlog'
import { createUserAgentEnricher, createGeoEnricher } from 'evlog/enrichers'

initLogger({
  enrichers: [createUserAgentEnricher(), createGeoEnricher()],
})
```
::

## Enrich Context

每个 enricher 都接收一个包含以下字段的 `EnrichContext`：

| 字段 | 类型 | 描述 |
|------|------|------|
| `event` | `WideEvent` | 发出的广泛事件（可变） |
| `request` | `object` | 请求元数据（`method`、`path`、`requestId`） |
| `headers` | `object` | 安全的 HTTP 请求头（敏感头会被过滤） |
| `response` | `object` | 响应元数据（`status`、`headers`） |

::callout{icon="i-lucide-shield-check" color="success"}
**安全性：** 敏感头（如 `authorization`、`cookie`、`x-api-key` 等）会被自动过滤，永远不会传递给 enricher。
::

## 可用 Enrichers

::card-group
  :::card
  ---
  icon: i-lucide-monitor-smartphone
  title: 用户代理
  to: /enrichers/built-in#user-agent
  ---
  从用户代理头解析浏览器、操作系统和设备类型。
  :::

  :::card
  ---
  icon: i-lucide-map-pin
  title: 地理位置
  to: /enrichers/built-in#geo
  ---
  从平台头中提取国家、地区、城市和坐标。
  :::

  :::card
  ---
  icon: i-lucide-hard-drive
  title: 请求大小
  to: /enrichers/built-in#request-size
  ---
  从 Content-Length 头捕获请求和响应有效负载大小。
  :::

  :::card
  ---
  icon: i-lucide-route
  title: 跟踪上下文
  to: /enrichers/built-in#trace-context
  ---
  从 traceparent 头提取 W3C 跟踪上下文（traceId、spanId）。
  :::

  :::card
  ---
  icon: i-lucide-code
  title: 自定义
  to: /enrichers/custom
  ---
  为任何派生上下文编写自己的 enricher。
  :::
::

## 覆盖行为

默认情况下，enricher 会保留现有字段。如果应用程序代码已经设置了 `event.userAgent`，enricher 不会覆盖它。传递 `{ overwrite: true }` 可更改此行为：

```typescript [enricher-factory-options.ts]
createUserAgentEnricher({ overwrite: true })
```

::callout{icon="i-lucide-code" color="neutral"}
请参阅 [Next.js 指南](/frameworks/nextjs) 以了解实际实现。
::

## 下一步

- [内置 Enrichers](/enrichers/built-in) - 所有内置 enricher 的详细参考
- [自定义 Enrichers](/enrichers/custom) - 编写你自己的 enricher
