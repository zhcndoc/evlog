...
| 代码 | 含义 |
| --- | --- |
| `0` | 分数达到阈值，没有相对于基线的回归，或者两者都未被请求 |
| `1` | 分数低于 `--min-score`，相对于 `--baseline` 出现回归，或者扫描无法运行 |
| `2` | 使用错误 — 未知标志，或无效的 `--framework` |
...
每一个提升分数的拉取请求，都会把底线一起抬高。报告已经告诉你下一步值多少钱：`▲ 76 → 86 by fixing the 3 above`。
...
除非你传入 `--no-write`，否则每次运行都会将 `evlog.map.json` 写入项目根目录。它包含与 `--json` 相同的数据，并且是一个构建产物，因此请将其忽略：

```bash [.gitignore]
evlog.map.json
```

如果你希望在评审中看到差异，也可以改为跟踪它——这样拉取请求就会准确显示哪些入口点更改了 class，这也是一个值得讨论的点。不过要预期该文件每次运行都会变动，因为 `generatedAt` 每次都会改变。提交它也是 [`--baseline`](#the-ratchet---baseline) 用来进行比较的内容。
