...
**此列表尚未最终确定。** 十条规则是引擎的起点，而不是终点。规则会不断增加，现有规则对判定标准会变得更加精确，而更严格的规则可能会改变你没有修改过的代码的裁决。Id 和权重都有文档记录，因此变更始终可以解释，但不要将不同 CLI 版本之间的分数视为可比较。
::
...
规则每条都各自对应一个文件，并拥有自己的测试台，因此错误裁决可以在本地修复，而不必进行考古式排查。如果 `evlog map <file>` 声称某件你能看到的事情并不属实，那就是一个值得[提交 issue](https://github.com/evloghq/evlog/issues) 的 bug。请附上入口点的输出和 handler，修复就能落在单条规则中。
