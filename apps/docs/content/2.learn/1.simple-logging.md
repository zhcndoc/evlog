---
title: 简单日志
description: 使用 evlog 记录单条结构化消息，使用与广泛事件相同的排水管道和脱敏功能，并调整现有日志记录器调用位置
navigation:
...
- [广泛事件](/learn/wide-events)：累积上下文并发出完整事件
- [结构化错误](/learn/structured-errors)：抛出带有 `why`、`fix` 和 `link` 的错误
- [配置](/reference/configuration)：所有 `initLogger` 选项
- [适配器](/integrate/adapters/overview)：将事件发送到 Axiom、Sentry、PostHog 等
- [Standalone TypeScript](/integrate/frameworks/standalone)：无需 Web 框架的脚本、工作进程和库
- [evlog 与其他日志记录器的对比](/reference/vs-other-loggers)：与 pino、winston、consola 并排对比
