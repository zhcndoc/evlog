---
title: 简单日志记录
description: evlog 的通用日志记录器，可直接替代 console.log、pino 或 consola，提供与广泛事件相同的级别、排水管道、脱敏和输出
navigation:
...
`log` API 是 evlog 的通用日志记录器。使用方式与 pino、consola 或 `console.log` 相同：每次调用都会通过与广泛事件相同的排水管道发出结构化事件。这两种模式可以共存；两者互不构成升级关系。

::callout{icon="i-lucide-globe" color="neutral"}
在 CLIs、库、任务和边缘环境中寻找相同的 API？请从 [Standalone TypeScript](/integrate/frameworks/standalone) 和 [Cloudflare Workers](/integrate/frameworks/cloudflare-workers) 开始。
::
...
可以使用 [Vite 插件](/reference/vite-plugin) 或 Nuxt 模块的 `strip` 选项，从生产构建中剥离 `log.debug()` 调用。
::
...
当使用对象形式时，事件会像广泛事件一样通过 [排水管道](/integrate/adapters/overview) 发送：
...
这四种方式都会变成下面这样，无需配置格式化器、传输器或 peer-dep：

```typescript [迁移后（evlog）]
import { initLogger, log } from 'evlog'
...
`initLogger` 在启动时只需一行。排水管道、脱敏、采样、美观格式／JSON 切换和级别过滤默认都会完成配置：无需 `pino-pretty` peer dep，无需组装 winston 传输器，也无需设置 consola reporter。

::callout{icon="i-lucide-arrow-right" color="neutral"}
想查看完整的并排对比（功能比较表、真实差距、逐项映射）？请参阅 [evlog vs pino, winston, consola](/reference/vs-other-loggers)。
::

## 与广泛事件配对使用

`log` 和 `createLogger` 位于同一个日志记录器中。对于独立存在的事件（启动消息、临时警告、调试跟踪），使用 `log.*`；当你希望一个事件捕获整个操作时，使用 `createLogger`。它们共享全局排水管道、脱敏和类型，因此可以按调用选择。
...
- [Wide Events](/learn/wide-events)：累积上下文并发出完整事件
- [Structured Errors](/learn/structured-errors)：抛出带有 `why`、`fix` 和 `link` 的错误
- [Configuration](/reference/configuration)：所有 `initLogger` 选项
- [Adapters](/integrate/adapters/overview)：将事件发送到 Axiom、Sentry、PostHog 等
- [Standalone TypeScript](/integrate/frameworks/standalone)：无需 Web 框架的脚本、工作进程和库
- [evlog vs other loggers](/reference/vs-other-loggers)：与 pino、winston、consola 并排对比
