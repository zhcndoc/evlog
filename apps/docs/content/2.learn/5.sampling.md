...
在我的 evlog 生产配置中启用头部和尾部采样。

- 识别我的框架并定位 evlog 配置（nuxt.config.ts、lib/evlog.ts、initLogger 等）
- 配置每个级别的 sampling.rates：{ info: 10, warn: 50, debug: 0, error: 100 } 作为起点
- 添加 sampling.keep 规则以强制保留关键事件：[{ status: 400 }, { duration: 1000 }, { path: '/api/critical/**' }]
- 对于业务特定的保留逻辑（例如付费用户），添加自定义 keep 回调或 evlog:emit:keep 钩子
- 将采样包裹在 $production 覆盖中，以便开发环境保留完整日志
- 确认除非我明确设置 error: 0，否则错误默认总是保留

文档：https://www.evlog.dev/learn/sampling
...
- [最佳实践](/reference/best-practices) - 安全和生产检查清单
