---
title: 生命周期
description: evlog 事件从创建到传输，贯穿三种模式，以及采样、丰富化和传输过程
navigation:
...
在 **`emit`** 之后（包括采样返回无输出时），请求日志记录器会被**封闭**：后续的 `set` / `error` / `info` / `warn` 调用会被忽略，并在控制台显示警告。对于需要创建独立事件的后台工作，请在你的集成支持时使用 **`log.fork()`**。请参阅宽事件中的[发送后](/learn/wide-events#after-emit-sealing-and-background-work)。
