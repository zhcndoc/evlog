...
在 **`emit`** 之后（包括采样返回无输出时），请求日志记录器会被**封存**：后续的 `set` / `error` / `info` / `warn` 调用会被忽略，并伴随控制台警告。对于需要自己事件的后台工作，请在你的集成支持的情况下使用 **`log.fork()`**。参见 [宽事件 — 发送后](/learn/wide-events#after-emit-sealing-and-background-work)。

## 请求日志 — 分步说明

对于由框架管理的请求日志，每个请求都会走完上面的流水线。下面详细说明每个阶段。
...
- [宽事件](/learn/wide-events) - 设计有效的宽事件
- [采样](/learn/sampling) - 配置头部和尾部采样
- [适配器](/integrate/adapters/overview) - 将事件发送到外部平台
- [丰富器](/use-cases/enrichers) - 自动添加派生上下文
