---
title: 目录
description: "将类型化错误和审计目录从一个文件扩展到 monorepo：约定、npm 打包方案和类型增强。"
navigation:
...
export const errors = defineErrorCatalog('app', {
  USER_NOT_FOUND: { status: 404, message: '未找到用户' },
  FORBIDDEN: { status: 403, message: '禁止访问' },
  VALIDATION_FAILED: {
    status: 400,
    message: ({ field }: { field: string }) => `无效的 ${field}`,
  },
})
```
...
export const billingPaymentErrors = defineErrorCatalog('billing.payment', {
  DECLINED: { status: 402, message: '银行卡被拒绝' },
  INSUFFICIENT_FUNDS: { status: 402, message: '余额不足' },
  EXPIRED_CARD: { status: 402, message: '银行卡已过期' },
  CVV_MISMATCH: { status: 402, message: 'CVV 不匹配' },
})
```
...
`declare module` 块位于源文件中，因此打包器会将其输出到 `dist/index.d.ts`。任何从 `@acme/errors-billing` 导入内容的消费者都会传递性地获得该增强，无需在其一侧进行额外设置。
...
**每个共享包拥有自己的前缀。** `@acme/errors-billing` 拥有 `billing.*`，`@acme/errors-auth` 拥有 `auth.*`。通过设计即可避免冲突。将目录升级到新的 minor 版本（添加条目）后，会通过常规的 semver 升级路径传播给消费者：无需代码生成，也无需迁移步骤。
::
...
集中式和分散式都可行，因为 TypeScript 会自动合并跨文件的多个 `declare module 'evlog'` 块。
...
`_codes` 字面量联合类型会生成实际的 `ErrorCode` 类型。键名本身是任意的，请根据你的结构选择合适的命名方式。
...
::warning
**绝不要在调用点覆盖 `code`。** 目录定义了 code 的身份，覆盖它会破坏仪表盘、告警以及根据 `err.code` 进行分支处理的消费者代码。工厂的调用点签名会特意从可覆盖字段中排除 `code`。
::
