...
::tip
For more than a handful of actions, prefer [`defineAuditCatalog`](/logging/audit/recording#defineauditcatalog) over a plain object dictionary — same `noun.verb` convention, but you get autocomplete on the action union and per-entry `target` type inference for free.
::
