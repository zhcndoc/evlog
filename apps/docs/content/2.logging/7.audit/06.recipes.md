...
| 符号 | 类型 | 说明 |
|---|---|---|
| `AuditFields` | type | wide event 上保留的字段 |
...
| `audit(fields)` | function | 供脚本 / 作业单独使用 |
| `withAudit({ action, target })(fn)` | wrapper | 自动发出 success / failure / denied |
| `auditDiff(before, after)` | helper | 对 `changes` 进行感知脱敏的 JSON Patch |
| `mockAudit()` | test util | 在测试中捕获并断言审计 |
| `auditEnricher(opts?)` | enricher | 自动填充 request / runtime / tenant 上下文 |
| `auditOnly(drain, { await? })` | wrapper | 仅路由带有 `audit` 字段的事件 |
| `signed(drain, opts)` | wrapper | 通用完整性包装器（hmac / hash-chain） |
| `auditRedactPreset` | config | 审计事件的严格 PII 脱敏预设 |
