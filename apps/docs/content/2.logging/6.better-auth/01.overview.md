...
文档：https://www.evlog.dev/logging/better-auth/overview
...
| 导出 | 描述 |
|--------|-------------|
| `identifyUser(log, session)` | 核心辅助函数——从会话中提取安全字段并设置到 logger 上。若已识别则返回 `true`。 |
| `createAuthMiddleware(auth)` | 返回一个带路由过滤、计时和 hooks 的异步 `(log, headers, path?) => Promise<boolean>` 函数。 |
| `createAuthIdentifier(auth)` | 适用于独立 Nitro 应用的 Nitro `request` hook 工厂。参见 [性能](/logging/better-auth/performance#standalone-nitro)。 |
