...
Use structured errors with why / fix / link fields throughout my app.

- Replace plain `throw new Error(...)` calls with createError({ message, status, why, fix, link })
...
- On the client, use parseError(err) to extract { message, status, why, fix, link } from any thrown error
