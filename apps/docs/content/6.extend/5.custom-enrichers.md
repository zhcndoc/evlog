...
- 通过 `enrich` 选项（中间件）或 `initLogger.enrichers`（独立模式）把 enricher 接入你的框架
...
为每个事件添加部署元数据。这个增强器在任何地方都是同一个函数——只是不同框架的接入步骤不同。
...
- [内置丰富器](/use-cases/enrichers) — 用户代理、地理位置、请求大小、跟踪上下文
- [插件](/extend/plugins) — 多钩子扩展（把 drain + enrich + keep 放在一个对象中）
- [适配器](/integrate/adapters/overview) — 将已丰富的事件发送到外部服务。
