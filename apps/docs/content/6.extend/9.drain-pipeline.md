...
- 对于多个目标，编写一个在内部进行分发的单一 drain 函数，使用 `Promise.allSettled([drainA(batch), drainB(batch)])`，然后将其传递给 `pipeline(...)`。一个共享缓冲区，一个共享重试预算
- 关闭时调用 `drain.flush()` 以发送缓冲的事件（Nitro 使用 `close` hook，Standalone 在 `process.exit` 前调用）。在 serverless 环境中，将流水线作为 `drain` 选项传递即可：中间件会向运行时的 `waitUntil` 注册 `drain.settled()`
- 在生产环境中始终使用流水线。直接使用 drain 会为每个事件发起一次 HTTP 请求，并且很快就会崩溃
...
始终在进程退出前刷新流水线（`drain.flush()`）。在 Nitro 中使用 `close` hook；在 standalone 脚本中于 `process.exit` 前调用。在 serverless 运行时中，中间件会为你延长实例的生命周期，参见 [Serverless](#serverless)。
::

## 流水线对事件的处理方式

事件到达 `evlog:drain` 后会被缓冲。当达到 `batch.size` 或 `batch.intervalMs` 到期时（以先发生者为准），批次会被刷新。失败时，相同批次会按照配置的退避策略重试；一旦 `retry.maxAttempts` 耗尽，就会使用丢失的事件调用 `onDropped`（如果没有配置该回调，则记录丢弃日志）。缓冲区大小受 `maxBufferSize` 限制。缓冲区填满后，会丢弃最旧的事件，以保持内存占用稳定。

### 流水线负责重试

直接调用时，Adapter drains（`createAxiomDrain()`、`createDatadogDrain()` 等）会吞掉传输失败，因此失败的后端永远不会中断请求。在流水线内部，这会使 `retry` 和 `onDropped` 成为无效代码，因此流水线会将适配器解包为会抛出错误的 `raw` 版本：失败会进入重试循环，而原始传输默认只进行一次 HTTP 尝试，以避免批次被重复重试。如果你明确希望在每次流水线尝试中额外进行 HTTP 尝试，请仅在适配器配置中设置 `retries`。
...
请参阅完整的 [browser example](https://github.com/evloghq/evlog/tree/main/examples/browser)，了解可运行的 Hono server + browser page。
