...
做出了很棒的东西？[提交 PR](https://github.com/hugorcd/evlog/pulls) 为适配器表添加一行——社区会感谢你。
::

## 下一步

- [Drain 管道](/extend/drain-pipeline) — 将你的 drain 包装进 batch + retry + fanout，用于生产环境
- [适配器概览](/integrate/adapters/overview) — 查看内置适配器如何使用 `defineHttpDrain`
- [自定义增强器](/extend/custom-enrichers) — 适用于派生事件字段的相同工具包形状
- [自定义框架集成](/extend/custom-framework) — 适用于 HTTP 框架的相同工具包形状
- [最佳实践](/reference/best-practices) — 安全与生产环境提示
