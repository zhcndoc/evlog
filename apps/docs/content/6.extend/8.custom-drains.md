...
- 在 `resolve()` 内通过 `resolveAdapterConfig(namespace, fields, overrides)` 惰性解析配置，这样用户就能获得标准优先级（覆盖项 → `runtimeConfig.evlog.<ns>` → 环境变量）
...
### 一个 5 分钟示例 — 内部 Loki 排空

一个完整可工作的排空，只有 25 行，并且不需要外部配置助手：
