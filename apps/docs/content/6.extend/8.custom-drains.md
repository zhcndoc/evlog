...
就是这样。`defineHttpDrain` 会处理重试（默认 2 次）、超时（默认 5000ms）、错误隔离以及身份标头（`User-Agent: evlog/<version>` + `X-Evlog-Source: <name>`）。即使目标服务宕机，你的应用流水线仍会继续运行；而当你将 drain 包装在 [`createDrainPipeline`](/extend/drain-pipeline) 中时，流水线会通过 drain 的 `raw` 变体接管重试。
...
让 `send` 在失败时抛出异常。`defineDrain` 会处理隔离：直接调用会吞掉错误并使用你的 drain 名称记录日志，而返回函数上的 `raw` 变体会拒绝，这正是[流水线](/extend/drain-pipeline)的重试和 `onDropped` 观察失败的方式。自行吞掉错误的 `send` 会使你的 drain 无法重试。
...
构建了很棒的东西？[提交 PR](https://github.com/evloghq/evlog/pulls)，为适配器表格添加一行。社区会感谢你。
::

## 下一步

- [Drain 流水线](/extend/drain-pipeline)：在生产环境中将你的 drain 包装在批处理 + 重试 + 扇出中
- [适配器概览](/integrate/adapters/overview)：了解内置适配器如何使用 `defineHttpDrain`
- [自定义 Enricher](/extend/custom-enrichers)：用于派生事件字段的相同工具包形状
- [自定义框架集成](/extend/custom-framework)：用于 HTTP 框架的相同工具包形状
- [最佳实践](/reference/best-practices)：安全和生产环境技巧。
