---
title: 身份标头
description: 每个 drain 请求都会携带 User-Agent 和 X-Evlog-Source，以便接收方对流量进行分类。需要时可以覆盖或抑制它们
navigation:
  title: 身份标头
...
浏览器端的 `evlog/http` drain（用于客户端日志记录）会改为设置 `X-Evlog-Source: client`，因为浏览器无法覆盖 `User-Agent`
...
- [Drain 流水线](/extend/drain-pipeline): 在保留身份标头的同时，将任意 drain 包装在批处理 + 重试中
