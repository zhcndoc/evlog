---
title: 食谱
description: 具体的可复制粘贴方案——构建你自己的最小开发工具，将数据通过管道传递给 curl + jq，先回放历史记录，再切换到实时模式，并在消费端进行聚合。
navigation:
  title: 食谱
...
::prompt
---
description: 构建一个自定义的 evlog 开发工具 / 仪表盘
icon: i-lucide-chef-hat
actions:
  - copy
  - cursor
...
启动一个本地开发工具或仪表盘，用来消费 evlog 宽事件。

- 选择来源：实时（通过 SSE 的流服务器）或历史记录（来自 `.evlog/logs` 的 `readFsLogs`），或两者兼用（先回放，再实时跟踪）
- 对于 SSE：通过 `.evlog/stream.url` 或 `GET /api/_evlog/stream-info` 发现 URL，绝不要硬编码端口
- 打开一个 `EventSource`，并将消息解码为 `{ evlog: '1', type, data }` 信封（`type` 为 `hello | event | replay | ping`）
- 对于运行在与开发服务器不同源上的浏览器标签页，通过流服务器的 `cors` 选项配置 CORS，并谨慎转发凭据
- 在消费端进行聚合（计数、延迟直方图、错误分组），并让服务器保持简单
- 在无服务器平台上跳过。该流在进程内运行
...
实时事件面板本质上就是 `EventSource` + 列表。完整的线协议和发现规则（`.evlog/stream.url`、`/api/_evlog/stream-info`、`{ evlog: '1', type, data }` 信封以及身份验证）记录在[流页面](/extend/stream#wire-format)中。下面的每个方案都假定你已经通过其中一种机制获取了 URL。
...
无需对正在运行的应用进行插桩即可工作，适合监视某个目录的 sidecar / observer 进程。

## 哪些情况会导致消费者失效

- **不要在 Vercel Functions / Cloudflare Workers / Lambda 上运行流服务器。** 每次调用都是一个独立的隔离环境；一个隔离环境中的订阅者永远看不到其他隔离环境发出的事件。对于跨实例扇出，请使用真正的 broker（Redis Streams、NATS、Pub/Sub）。
- **除非你的 evlog 配置会对数据进行脱敏，否则不要将涉及身份验证的数据放入宽事件中。** 服务器会原样转发应用发出的内容，包括任何未脱敏的 PII。
- **不要在服务器端进行过滤**（“只要错误事件”）。服务器的设计目标就是保持透明。在消费端进行过滤；这样一个过滤条件就不会让另一个消费者收不到事件。

以上每一点都是[流](/extend/stream)的属性，而不是消费者的属性：一个进程内总线、一个 SSE 桥接，以及一个会回放但永远不会持久化到磁盘的环形缓冲区。
