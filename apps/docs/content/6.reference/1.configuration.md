...
| `env` | `Partial<EnvironmentContext>` | Auto-detected | 环境上下文覆盖（见下文） |
| `pretty` | `boolean` | `true` in dev | 采用树状格式美观打印。根据 `NODE_ENV` 自动检测 |
| `silent` | `boolean` | `false` | 抑制控制台输出。事件仍会被构建、采样并传递给排水适配器 |
| `stringify` | `boolean` | `true` | 当禁用 `pretty` 时输出 JSON 字符串。Cloudflare Workers 请设为 `false` |
| `minLevel` | `'debug' \| 'info' \| 'warn' \| 'error'` | `'debug'` | 仅适用于全局 `log` API 的最低严重级别（不适用于 `createLogger` / 请求级宽事件）。顺序：debug < info < warn < error |
| `sampling` | `SamplingConfig` | `undefined` | 头部和尾部采样配置。见 [采样](/learn/sampling) |
| `redact` | `boolean \| RedactConfig` | `true` in production | 默认在生产环境启用。设为 `false` 可禁用。对象用于细粒度控制。见 [自动脱敏](/learn/redaction) |
...
**Nuxt and Nitro** 使用模块配置和 Nitro 钩子（`evlog:drain`、`evlog:enrich`、`evlog:emit:keep`）而不是中间件选项。请参见 [Nuxt](/integrate/frameworks/nuxt) 和 [Nitro](/integrate/frameworks/nitro) 页面。
::
