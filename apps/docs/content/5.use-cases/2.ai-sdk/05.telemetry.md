...
`createAILogger` 会记录 token、模型信息和流式指标。若需要更深入的可观测性（按工具统计执行耗时、成功／失败跟踪以及生成的总墙钟时间），请在此基础上添加 `createEvlogIntegration()`。它实现了 AI SDK 遥测接口（v6 `TelemetryIntegration`、v7 `Telemetry`），并能捕获仅凭数据中间件无法看到的数据。

在 AI SDK v7 中，该集成还会通过 `onEmbedEnd` 自动捕获嵌入，通过 `onAbort` 记录流中止，并通过 `onError` 提供无法恢复的错误。
...
| 数据 | 来源 | 描述 |
|------|--------|-------------|
| `ai.tools[]` | `onToolExecutionEnd` (v7) / `onToolCallFinish` (v6) | 每个工具的 `name`、`durationMs`、`success`，以及失败时的 `error` |
| `ai.totalDurationMs` | `onStart` → `onEnd` (v7) / `onFinish` (v6) | 从生成开始到完成的总墙钟时间 |
| `ai.embedding` | `onEmbedEnd` (v7) | 启用遥测后，自动从 `embed()` / `embedMany()` 中捕获 |
| `ai.finishReason: 'abort'` | `onAbort` (v7) | 流式生成中止时设置 |
| `ai.error` | `onAbort` / `onError` (v7) | 中止原因或无法恢复的生成错误 |
