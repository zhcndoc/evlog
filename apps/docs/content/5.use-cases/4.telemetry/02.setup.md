...
| --- | --- | --- |
| `my-tool doctor` | `doctor` | `{ json: true }` —— 布尔值会作为值被捕获 |
| `my-tool doctor --json` | `doctor` | `{ json: true }` |
| `my-tool sync --dry-run --output ./out` | `sync` | `{ dryRun: true, output: true }` —— 字符串路径只记录**是否存在** |
| `my-tool sync --format json` | `sync` | 仅在 `collect.flags` 白名单中时才会记录 `{ format: "json" }` |
| `my-tool telemetry status` | `telemetry status` | 嵌套子命令会用空格连接 |
