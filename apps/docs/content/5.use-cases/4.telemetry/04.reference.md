...
- **位置参数** — citty 的 `_` 容器存放的是参数值，而不是标志名称
- **默认值** — 仍为声明的 `default` 值的标志不算作用户做出的选择
- **短横线命名法重复项** — `--min-score` 会同时以 `minScore` 和 `min-score` 的形式到达；只保留驼峰命名的名称，而这也是 `collect.flags` 允许列表中使用的名称

否定形式仍会被记录：对于 `write: { default: true }`，`--no-write` 会记录为 `write: false`。

在同一个 `withTelemetry()` / `createTelemetry()` 调用中声明与 `collect` 一起使用的允许列表 — 无需单独的配置文件。
