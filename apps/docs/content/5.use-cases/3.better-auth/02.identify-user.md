...
::tip
**默认安全。** 只提取列入白名单的字段，密码、令牌和机密信息绝不会写入日志记录器。
::
...
::tip
请确保 `extend` 是确定性的，因为它会在每个请求中运行。避免在其中执行繁重的计算或额外的数据库调用；应查询 Better Auth 已经加载到会话中的数据。
::
...
| `auth.resolvedIn` | 已测量 | 会话解析耗时，单位为 ms。 |
| `auth.identified` | 已计算 | 请求是否已被识别。 |
