...
description: 在经过身份验证的请求中识别用户，这样每个 wide 事件都会携带发起者信息，无需手动调用 log.set。
navigation:
  title: 概览
  icon: i-lucide-list
links:
...
`evlog/better-auth` 会将匿名 wide 事件转换为已识别事件。每个请求都会自动包含发起者信息，无需手动调用 `log.set({ user })`。

## 接入前

在你的应用中将 [Better Auth](https://better-auth.com/) 作为**直接依赖**使用。`evlog` 不会捆绑 Better Auth。该集成已针对 Better Auth `>=1.6.9` 进行测试（与[ playground](https://github.com/evloghq/evlog/tree/main/apps/playground)使用相同的主版本）。
...
- 在中间件／hook 中调用 identify(log, headers, path)，以便在每个请求中自动识别用户
- 默认安全。仅提取列入白名单的字段，并且绝不会记录密码或令牌
- 支持 include／exclude 路由模式、生命周期 hook 和 Better Auth 插件字段
- 适用于所有框架：Nuxt、Next.js、Express、Hono、Fastify、NestJS、Elysia，以及独立应用

文档：https://www.evlog.dev/use-cases/better-auth/overview
...
::card-group
  ...
  to: /use-cases/better-auth/identify-user
  ---
  核心构建模块：提取安全字段、隐藏邮箱，并捕获插件数据（组织、角色、2FA）。
  :::
  ...
  to: /use-cases/better-auth/middleware
  ---
  使用 `include`／`exclude` 过滤路由，通过生命周期 hook 响应识别，并按应用调优行为。
  :::
