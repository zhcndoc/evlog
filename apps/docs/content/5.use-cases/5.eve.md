---
title: eve
description: "每个 eve agent 轮次产生一个 evlog 宽事件：令牌使用量、工具执行和业务上下文，通过你自己的 drain 和尾部采样实现。"
navigation:
  title: eve
  icon: i-custom-eve
links:
...
[eve](https://eve.dev/docs/introduction) 内置了可观测性：Vercel 上的 **Agent Runs**，以及通过 `agent/instrumentation.ts` 提供的可选 **OpenTelemetry** spans。`evlog/eve` 增加了第三层：每个轮次都生成**可导出的宽事件**，并接入你完整的 evlog 管道（drains、enrichers、尾部采样、审计）。
...
- 在工具中从 'evlog/eve' 导入 useLogger，并在 execute() 内调用 useLogger()。注册 defineEvlogHook() 后，轮次 logger 会通过 AsyncLocalStorage 绑定；只有在运行时无法使用 ALS 时才传入 ctx
- 默认省略用户消息内容（message: 'omit'）；只有在检查 PII 策略后才使用 'preview' 或 'full'
- 可选：添加 agent/instrumentation.ts，并使用来自 'evlog/eve' 的 defineEvlogInstrumentation 将 OTel spans 关联到宽事件
...
使用 `eve/next` 中的 `withEve()` 包装 `next.config.ts`，并在应用中使用 `eve/react` 中的 `useEveAgent()`。eve 会与 `next dev` 一起启动，并在同一源上代理 `/eve/v1/*`，因此工具调用、审批和 `ask_question` 都可以直接使用。请参阅[示例 agent](https://github.com/HugoRCD/evlog/tree/main/examples/eve)。
...
这样，轮次就会进入与其生成的 LLM 追踪相同的 PostHog 用户。请参阅 [PostHog 适配器](/integrate/adapters/cloud/posthog#link-a-log-to-a-person-and-a-session-replay)。
...
- [eve hooks 指南](https://eve.dev/docs/guides/hooks)：流事件词汇
- [eve instrumentation](https://eve.dev/docs/guides/instrumentation)：OTel spans（互补功能）
- [AI SDK 用例](/use-cases/ai-sdk/overview)：当你直接拥有模型循环时
- [适配器概览](/integrate/adapters/overview)：drain 目标站点
