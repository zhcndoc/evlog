...
::tip
**`action` 的命名约定。** 使用 `noun.verb`（`invoice.refund`、`user.invite`、`apiKey.revoke`）。如果动作已经发生，则使用过去时（`invoice.refunded`）；如果 `withAudit()` 将解析结果，则使用现在时。将一个小型固定字典放在一个文件中，因为审计员和 SIEM 规则会查询 `action`，所以拼写错误就意味着告警缺失
::
...
::tip
如果动作数量不止几个，与其使用普通对象字典，不如使用 [`defineAuditCatalog`](/use-cases/audit/recording#defineauditcatalog)：同样遵循 `noun.verb` 约定，但你可以获得动作联合类型的自动补全，以及每个条目的 `target` 类型推断
::
...
`'failure'` 和 `'denied'` 是两种不同的情况，审计员非常关注被拒绝的动作，因为它们可能意味着探测行为或访问控制配置错误。始终记录拒绝事件（请参阅[记录事件](/use-cases/audit/recording#logauditdeny)）。

## 确保重试安全

`idempotencyKey` 根据 `action`、`actor.id`、`target` 和粗粒度时间戳的哈希自动派生。结果是，即使你的 drain 在网络短暂中断后重试审计插入，重复行也会在 `ON CONFLICT DO NOTHING` 时合并。你不必考虑这一点，因为该字段会自动为你填充。
