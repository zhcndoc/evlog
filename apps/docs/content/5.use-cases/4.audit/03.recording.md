---
title: 记录事件
description: log.audit 和 log.audit.deny、独立 audit()、withAudit、defineAuditAction 和 defineAuditCatalog 注册表，以及 auditDiff
navigation:
  title: 记录
...
::note
独立 `audit()` 事件没有 `requestId`、`context.ip` 或 `userAgent`，因为没有可供其丰富信息的请求。当这对取证很重要时，请手动添加你自己的上下文（`context: { jobId, queue, runId }`）。
::
...
将此与 [架构 → 动作命名](/use-cases/audit/schema#action-naming) 中的动作字典配合使用。
...
### `defineAuditAction` 与 `defineAuditCatalog`：何时选择

二者都会生成相同的调用处工厂形状。按规模选择：

- **`defineAuditAction(action, opts?)`**：一次性动作，或在非常大的代码仓库中按文件组织动作。它对应 `defineError`。其效果等同于只有一个条目的 catalog，但不会推导前缀：你需要直接写出完整的 wire `action`。
- **`defineAuditCatalog(prefix, map)`**：将超出少量相关动作的内容归入同一个前缀下。它对应 `defineErrorCatalog`。wire `action` 会自动推导为 `${prefix}.${KEY}`，catalog 元数据（`_actions`、`_prefix`）会暴露出来以供自省，并且只需一行 `declare module 'evlog'`，即可将整个集合呈现在类型化的 `AuditAction` 联合类型中。

你可以在同一个代码库中混用二者。将跨领域的一次性动作保留为 `defineAuditAction`，将有界上下文（`billing`、`auth`、`subscription`）组织为 catalog。
...
::tip
**何时进行包装，何时手动调用。** 对于*纯粹值得审计的动作*（退款、删除、更改角色、重置密码），请使用包装函数：结果判定会自动完成，你也不会意外跳过调用。当审计只是更大处理程序中的多个决策之一，或者你需要在动作完成*之前*发出审计（例如“用户请求删除”）时，请坚持手动调用 `log.audit()`。
::
