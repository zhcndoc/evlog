...
不使用 `auditEnricher` 时，`audit.context` 会保持为空，而审计人员和事件响应人员至少需要 `requestId` 和 `ip` 才能对记录的操作进行交叉定位。

## `auditOnly()`

::tip
**为什么要将审计过滤到单独的 drain？** 有三个原因：**成本**（与产品遥测相比，审计量很小，因此将它们分开可以避免保留成本失控）、**权限**（审计数据集应对工程师只读，而应用只能写入）以及**保留期限**（审计通常需要保存 7 年以上，而产品日志很少保存超过 90 天）。
::
...
设置 `await: true` 可使审计写入同步执行（审计不会 fire-and-forget，因此默认可安全应对崩溃）：
...
::tip
**`signed()` 实际上能带来什么。** 检测，而非预防。任何拥有底层 drain 写入权限的人仍然可以删除文件或表，但该链可以在事后证明哪些事件被删除或修改过。如果你已经写入仅追加／WORM 存储（S3 Object Lock、具有行级不可变性的 Postgres、仅追加的 BigQuery 表），请跳过 `signed()`；重复添加完整性层只会增加延迟，却不会提高门槛。
::
