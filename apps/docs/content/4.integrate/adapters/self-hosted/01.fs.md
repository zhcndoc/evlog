...
FS 适配器需要 Node.js（`node:fs`）。在 Edge 运行时中，或其目录不可写时，它会记录一次 `[evlog/fs]` 警告并跳过写入——因此在无服务器主机上挂载它是安全的，但没有实际意义，因为那里只有临时目录可写，且其内容不会在实例生命周期结束后保留。对于这些场景，请使用 `evlog/memory` 或 HTTP 适配器。
::
```typescript [Hono]
import { createFsDrain } from 'evlog/fs'
