---
title: NuxtHub 存储
description: 使用 NuxtHub 数据库存储实现自托管保留。无需外部依赖即可存储、查询和清理宽事件。
navigation:
  title: NuxtHub
...
文档：https://www.evlog.dev/integrate/adapters/self-hosted/nuxthub
...
- [适配器](/integrate/adapters/overview) - 将日志发送到外部服务，同时使用 NuxtHub 存储
- [管道](/extend/drain-pipeline) - 对事件进行批处理，以获得更好的数据库性能。
