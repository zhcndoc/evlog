...
适配器文档：https://www.evlog.dev/integrate/adapters/cloud/hyperdx
...
底层上，`createHyperDXDrain()` 会将你的 HyperDX 设置映射到共享的 [OTLP 适配器](/integrate/adapters/cloud/otlp)，并调用 `sendBatchToOTLP()`：
...
- [OTLP 适配器](/integrate/adapters/cloud/otlp) - 通过 OpenTelemetry 协议将日志发送到任何 OTLP 后端
- [PostHog 适配器](/integrate/adapters/cloud/posthog) - 通过 OTLP 将日志发送到 PostHog Logs
- [自定义适配器](/extend/custom-drains) - 构建你自己的适配器
- [最佳实践](/reference/best-practices) - 安全性和生产环境建议
