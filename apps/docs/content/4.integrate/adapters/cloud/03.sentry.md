---
title: Sentry 适配器
description: 将结构化日志发送到 Sentry 日志，以支持高基数查询和调试。通过环境变量即可实现零配置设置。
navigation:
  title: Sentry
...
[Sentry](https://sentry.io) 是一个错误追踪与性能监控平台。evlog Sentry 适配器会将你的宽事件以 **Sentry 结构化日志** 的形式发送，在 Sentry 控制台的 **探索 > 日志** 中可见，并支持高基数可搜索属性。

::prompt
---
description: 添加 Sentry 排水器适配器
icon: i-simple-icons-sentry
actions:
  - copy
  - cursor
...
添加 Sentry 排水器适配器，将 evlog 宽事件发送到 Sentry 日志。
...
6. 通过触发请求进行测试，并在 Sentry > 探索 > 日志 中检查

适配器文档：https://www.evlog.dev/integrate/adapters/cloud/sentry
...
3. 在 **设置 > 项目 > [你的项目] > 客户端密钥（DSN）** 中找到你的 DSN
...
就这样！你的日志现在会出现在 Sentry 的 **探索 > 日志** 中。

## 配置

适配器按照优先级顺序从多个来源读取配置（优先级从高到低）：

1. **覆盖项** 传递给 `createSentryDrain()`
2. **运行时配置** 位于 `runtimeConfig.sentry`（仅限 Nuxt/Nitro）
3. **环境变量**（`SENTRY_*`）
...
| `SENTRY_ENVIRONMENT` | 环境名称覆盖项 |
| `SENTRY_RELEASE` | 发布版本覆盖项 |
...
- **Explore > 日志**：查看所有 evlog 宽事件，并支持完整属性搜索
...
你的 DSN 可能被吊销或无效。请在 **设置 > 项目 > 客户端密钥（DSN）** 中生成新的 DSN。
