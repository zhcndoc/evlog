...
| `recordShape` | `'json' \| 'compact'` | `'json'` | 日志记录形状，参见[下文](#choose-a-record-shape) |
...
- **身份验证**：`Authorization: Bearer {apiKey}` 请求头
- **格式**：标准 OTLP `ExportLogsServiceRequest`，包含严重级别、跟踪上下文和结构化属性
- **身份标识**：`posthogDistinctId` 和 `sessionId` 属性，PostHog 通过它们将日志与项目中的其他数据关联起来
...
| `recordShape` | `'json' \| 'compact'` | `'json'` | 事件属性形状：`compact` 会扁平化嵌套字段（参见[下文](#event-format)） |
| `timeout` | `number` | `5000` | 请求超时时间，单位为毫秒 |
...
使用 `recordShape: 'compact'` 时，其他字段会以与日志模式相同的方式扁平化为点号键：嵌套的 `ai` 对象会作为 `properties.ai.costUsd` 和 `properties.ai.calls` 到达，而不是作为一个不透明的 `properties.ai` 值。PostHog 可以对这些单独的属性进行过滤和细分；默认的 `json` 形状会将嵌套对象保留为单个序列化属性。有关其中的权衡，以及它如何影响已保存的视图和警报，请参见[选择记录形状](#choose-a-record-shape)。
...
当没有解析出标识符时，事件会作为**匿名事件**发送（`$process_person_profile: false`），而不是将每个请求都归入一个以服务命名的“用户”。PostHog 对匿名事件的收费更低，并且不会将其纳入用户个人资料。
