...
在 Better Stack 的控制面板中，这个凭据称为 **source token**。为保持与各适配器一致，evlog 将配置字段命名为 `apiKey`。旧版 `sourceToken` 字段在下一个大版本发布前仍然可用。
::
...
- [Axiom 适配器](/integrate/adapters/cloud/axiom) - 将日志发送到 Axiom 以便查询和查看仪表板
- [OTLP 适配器](/integrate/adapters/cloud/otlp) - 通过 OpenTelemetry 协议发送日志
