---
title: 适配器概述
description: 使用 evlog 适配器将日志发送到外部服务。内置支持流行的可观测性平台和自定义目标。
navigation:
  title: 概述
  icon: i-lucide-list
links:
  - label: Axiom
    icon: i-custom-axiom
    to: /integrate/adapters/cloud/axiom
    color: neutral
    variant: subtle
  - label: PostHog
    icon: i-simple-icons-posthog
    to: /integrate/adapters/cloud/posthog
    color: neutral
    variant: subtle
  - label: Sentry
    icon: i-simple-icons-sentry
    to: /integrate/adapters/cloud/sentry
    color: neutral
    variant: subtle
  - label: Better Stack
    icon: i-simple-icons-betterstack
    to: /integrate/adapters/cloud/better-stack
    color: neutral
    variant: subtle
  - label: Datadog
    icon: i-simple-icons-datadog
    to: /integrate/adapters/cloud/datadog
    color: neutral
    variant: subtle
  - label: Grafana Loki
    icon: i-simple-icons-grafana
    to: /integrate/adapters/hybrid/loki
    color: neutral
    variant: subtle
  - label: ClickHouse
    icon: i-simple-icons-clickhouse
    to: /integrate/adapters/hybrid/clickhouse
    color: neutral
    variant: subtle
  - label: OTLP
    icon: i-simple-icons-opentelemetry
    to: /integrate/adapters/hybrid/otlp
    color: neutral
    variant: subtle
  - label: HyperDX
    icon: i-custom-hyperdx
    to: /integrate/adapters/hybrid/hyperdx
    color: neutral
    variant: subtle
  - label: 文件系统
    icon: i-lucide-hard-drive
    to: /integrate/adapters/self-hosted/fs
    color: neutral
    variant: subtle
  - label: NuxtHub
    icon: i-simple-icons-nuxt
    to: /integrate/adapters/self-hosted/nuxthub
    color: neutral
    variant: subtle
  - label: 内存
    icon: i-lucide-cpu
    to: /integrate/adapters/self-hosted/memory
    color: neutral
    variant: subtle
---

适配器允许您将日志发送到外部可观测性平台。evlog 提供内置适配器以支持流行的服务，并且您可以为任何目标创建自定义适配器。

::drain-fan-out
::

## 在代码中无需命名目标地发送事件

适配器在每次请求完成后接收一个 `DrainContext`，并将数据发送到外部服务。排水过程以**即发即忘**模式运行，这意味着它永远不会阻塞 HTTP 响应。

如何连接适配器取决于您使用的框架：

::code-group
```typescript [Nuxt / Nitro]
// server/plugins/evlog-drain.ts
import { createAxiomDrain } from 'evlog/axiom'

export default defineNitroPlugin((nitroApp) => {
  nitroApp.hooks.hook('evlog:drain', createAxiomDrain())
})
```
```typescript [Next.js]
// lib/evlog.ts
import { createEvlog } from 'evlog/next'
import { createAxiomDrain } from 'evlog/axiom'

export const { withEvlog, useLogger, log, createError } = createEvlog({
  service: 'my-app',
  drain: createAxiomDrain(),
})
```
```typescript [Hono]
import { createAxiomDrain } from 'evlog/axiom'

app.use(evlog({ drain: createAxiomDrain() }))
```
```typescript [Express]
import { createAxiomDrain } from 'evlog/axiom'

app.use(evlog({ drain: createAxiomDrain() }))
```
```typescript [Fastify]
import { createAxiomDrain } from 'evlog/axiom'

await app.register(evlog, { drain: createAxiomDrain() })
```
```typescript [Elysia]
import { createAxiomDrain } from 'evlog/axiom'

app.use(evlog({ drain: createAxiomDrain() }))
```
```typescript [NestJS]
import { createAxiomDrain } from 'evlog/axiom'

EvlogModule.forRoot({ drain: createAxiomDrain() })
```
```typescript [独立模式]
import { createAxiomDrain } from 'evlog/axiom'

initLogger({ drain: createAxiomDrain() })
```
::

::callout{icon="i-lucide-cloud" color="info"}
**无服务器支持：** 在 Cloudflare Workers 和 Vercel Edge 上，evlog 会自动使用 `waitUntil()` 来确保在运行时终止前排水完成。无需额外配置。
::

## 选择目标

::card-group
  :::card
  ---
  icon: i-custom-axiom
  title: Axiom
  to: /integrate/adapters/cloud/axiom
  ---
  将日志发送到 Axiom，Axiom 会为每个字段建立索引，因此您可以针对任意字段进行查询。
  :::
  :::card
  ---
  icon: i-simple-icons-posthog
  title: PostHog
  to: /integrate/adapters/cloud/posthog
  ---
  将日志发送到 PostHog Logs，以便进行结构化日志记录和可观测性。
  :::
  :::card
  ---
  icon: i-simple-icons-sentry
  title: Sentry
  to: /integrate/adapters/cloud/sentry
  ---
  将结构化日志发送到 Sentry Logs，以便进行高基数查询。
  :::
  :::card
  ---
  icon: i-simple-icons-betterstack
  title: Better Stack
  to: /integrate/adapters/cloud/better-stack
  ---
  将日志发送到 Better Stack，以便进行日志管理和告警。
  :::
  :::card
  ---
  icon: i-simple-icons-datadog
  title: Datadog
  to: /integrate/adapters/cloud/datadog
  ---
  通过本地 HTTP 摄入 API 将日志发送到 Datadog Logs。
  :::
  :::card
  ---
  icon: i-simple-icons-grafana
  title: Grafana Loki
  to: /integrate/adapters/hybrid/loki
  ---
  将日志推送到自托管、多租户或 Grafana Cloud Loki。
  :::
  :::card
  ---
  icon: i-simple-icons-clickhouse
  title: ClickHouse
  to: /integrate/adapters/hybrid/clickhouse
  ---
  将日志插入 ClickHouse，以便对海量数据进行快速聚合。
  :::
  :::card
  ---
  icon: i-simple-icons-opentelemetry
  title: OTLP
  to: /integrate/adapters/hybrid/otlp
  ---
  面向 Grafana、Datadog、Honeycomb 等平台的 OpenTelemetry 协议。
  :::
  :::card
  ---
  icon: i-custom-hyperdx
  title: HyperDX
  to: /integrate/adapters/hybrid/hyperdx
  ---
  使用其文档中提供的摄入端点和 API 密钥，通过 OTLP/HTTP 将日志发送到 HyperDX。
  :::
  :::card
  ---
  icon: i-lucide-hard-drive
  title: 文件系统
  to: /integrate/adapters/self-hosted/fs
  ---
  将日志写入本地 NDJSON 文件，用于调试和 AI 代理集成。
  :::
  :::card
  ---
  icon: i-simple-icons-nuxt
  title: NuxtHub
  to: /integrate/adapters/self-hosted/nuxthub
  ---
  在您的 NuxtHub 数据库中进行自托管日志存储，并带有自动保留策略。
  :::
  :::card
  ---
  icon: i-lucide-cpu
  title: 内存
  to: /integrate/adapters/self-hosted/memory
  ---
  适用于任何运行时的内存环形缓冲区，包括 Cloudflare Workers。
  :::
  :::card
  ---
  icon: i-lucide-code
  title: 自定义
  to: /extend/custom-drains
  ---
  为任何目标构建您自己的适配器。
  :::
  :::card
  ---
  icon: i-lucide-globe
  title: HTTP
  to: /extend/drain-pipeline
  ---
  在不耦合框架的情况下将客户端日志发送到您的服务器。
  :::
  :::card
  ---
  icon: i-lucide-workflow
  title: 管道
  to: /extend/drain-pipeline
  ---
  批量处理事件、失败重试并处理缓冲区溢出。
  :::
::

## 在请求之外调用排水器

在纯 TypeScript 或 Bun 脚本（无 HTTP 框架）中使用 `initLogger` 中的 `drain` 选项。每个发出的事件都会自动被排出。

```typescript [index.ts]
import type { DrainContext } from 'evlog'
import { initLogger, log, createRequestLogger } from 'evlog'
import { createAxiomDrain } from 'evlog/axiom'
import { createDrainPipeline } from 'evlog/pipeline'

const pipeline = createDrainPipeline<DrainContext>()
const drain = pipeline(createAxiomDrain())

initLogger({
  env: { service: 'my-script' },
  drain,
})

log.info({ action: 'job_started' }) // 自动排出

const reqLog = createRequestLogger({ method: 'POST', path: '/process' })
reqLog.set({ processed: 42 })
reqLog.emit() // 自动排出

await drain.flush()
```

::callout{icon="i-lucide-arrow-right" color="neutral"}
请参阅完整的 [bun-script 示例](https://github.com/hugorcd/evlog/tree/main/examples/bun-script)，了解一个实际的批处理脚本。
::

## 同时发送到多个位置

通过组合排水器同时将日志发送到多个服务：

```typescript [src/index.ts]
import { createAxiomDrain } from 'evlog/axiom'
import { createOTLPDrain } from 'evlog/otlp'

const axiom = createAxiomDrain()
const otlp = createOTLPDrain()

const drain = async (ctx) => {
  await Promise.allSettled([axiom(ctx), otlp(ctx)])
}
```

然后将 `drain` 传递给您的框架：

::code-group
```typescript [Nuxt / Nitro]
// server/plugins/evlog-drain.ts
export default defineNitroPlugin((nitroApp) => {
  nitroApp.hooks.hook('evlog:drain', drain)
})
```
```typescript [Next.js]
// lib/evlog.ts
import { createEvlog } from 'evlog/next'

export const { withEvlog, useLogger, log, createError } = createEvlog({
  service: 'my-app',
  drain,
})
```
```typescript [Hono]
app.use(evlog({ drain }))
```
```typescript [Express]
app.use(evlog({ drain }))
```
```typescript [Fastify]
await app.register(evlog, { drain })
```
```typescript [Elysia]
app.use(evlog({ drain }))
```
```typescript [NestJS]
EvlogModule.forRoot({ drain })
```
```typescript [独立模式]
initLogger({ drain })
```
:::

## 排水器接收的内容

每个适配器都会接收一个带有以下内容的 `DrainContext`：

| 字段 | 类型 | 描述 |
|------|------|------|
| `event` | `WideEvent` | 包含所有累积上下文的完整日志事件 |
| `request` | `object` | 请求元数据（`method`、`path`、`requestId`） |
| `headers` | `object` | 安全的 HTTP 头（敏感头会被过滤） |

::callout{icon="i-lucide-shield-check" color="success"}
**安全性：** 敏感头信息（如 `authorization`、`cookie`、`x-api-key` 等）会自动被过滤，不会传递给适配器。
::

## 从环境中进行配置

所有适配器都支持通过环境变量自动配置，无需更改代码即可在不同环境中部署。

每个适配器都会读取标准环境变量，并且相同的名称可在每个框架中使用：

```bash [.env]
# Axiom
AXIOM_API_KEY=xaat-xxx
AXIOM_DATASET=my-logs

# OTLP
OTLP_ENDPOINT=https://otlp.example.com

# HyperDX
HYPERDX_API_KEY=<YOUR_HYPERDX_API_KEY_HERE>

# PostHog
POSTHOG_API_KEY=phc_xxx

# Sentry
SENTRY_DSN=https://key@o0.ingest.sentry.io/123

# Better Stack
BETTER_STACK_API_KEY=your-source-token

# Datadog
DD_API_KEY=your-api-key
DD_SITE=datadoghq.eu

# Grafana Loki
LOKI_ENDPOINT=http://localhost:3100
# 仅适用于 Grafana Cloud：
LOKI_USER=123456
LOKI_API_KEY=glc_xxx

# ClickHouse
CLICKHOUSE_ENDPOINT=http://localhost:8123
CLICKHOUSE_PASSWORD=your-password
```

适配器会自动从这些变量中读取配置，因此 `create*Drain()` 工厂方法无需传入参数。

## 缺少凭据

行为取决于您调用的是哪个 API：

| API | 缺少凭据 |
|-----|----------|
| `create*Drain()`（工厂方法） | `console.error` + 排水器变为无操作 —— HTTP 响应绝不会被阻塞 |
| `sendTo*` / `sendBatchTo*`（直接调用） | 如果缺少必需凭据则抛出错误 —— 调用方必须传入完整配置 |

这种拆分是有意为之：排水器在每个请求后以 fire-and-forget 方式运行；直接发送辅助函数则用于脚本和测试，在这些场景中，静默失败会掩盖配置错误。
