---
title: HyperDX 适配器
description: 通过 OTLP/HTTP，使用其文档中说明的端点和授权标头，将广泛事件发送到 HyperDX。通过环境变量进行配置。
navigation:
  title: HyperDX
...
HyperDX 是开源的，因此适配器使用相同的代码，同时支持托管账户和你自行运行的集群。只有 `endpoint` 会发生变化。
...
将 `endpoint` 指向你自己的 OTLP HTTP 收集器，与在 `otlphttp` 导出器的 `endpoint` 中填写的值相同：
...
填写**基础** URL，而不是信号路径。evlog 会自行追加 `/v1/logs`。默认 OTLP HTTP 端口上的自托管收集器地址为 `http://host:4318`。
::
...
参考 [HyperDX OpenTelemetry 设置](https://hyperdx.io/docs/install/opentelemetry)：
