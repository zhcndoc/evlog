...
添加 HyperDX 排水适配器，以将 evlog 广泛事件发送到 HyperDX。
...
适配器文档：https://www.evlog.dev/integrate/adapters/hybrid/hyperdx
...
就是这样！现在你的宽泛事件将出现在 HyperDX 中。

## 配置

适配器会从多个来源读取配置（优先级从高到低）：

1. **覆盖项** 传递给 `createHyperDXDrain()`
2. **运行时配置** 位于 `runtimeConfig.evlog.hyperdx` 或 `runtimeConfig.hyperdx`（仅限 Nuxt/Nitro）
3. **环境变量**（`HYPERDX_*`）
...
| 变量 | 描述 |
|----------|-------------|
| `HYPERDX_API_KEY` | 接收摄取 API 密钥（作为 `authorization` 请求头发送） |
| `HYPERDX_OTLP_ENDPOINT` | OTLP HTTP 基础 URL（默认值：`https://in-otel.hyperdx.io`） |
| `HYPERDX_SERVICE_NAME` | 覆盖 `service.name` |
...
export default defineNuxtConfig({
  runtimeConfig: {
    hyperdx: {
      apiKey: '', // 通过 HYPERDX_API_KEY 设置
      // endpoint: '', // 通过 HYPERDX_OTLP_ENDPOINT 设置
    },
  },
})
```
...
HyperDX 是开源的，因此适配器可以使用同一套代码连接托管账户和你自行运行的集群——只需更改 `endpoint`。
...
端点默认为 `https://in-otel.hyperdx.io`。

### 自托管

将 `endpoint` 指向你自己的 OTLP HTTP 收集器——与在 `otlphttp` 导出器中设置的 `endpoint` 相同：
...
请提供**基础** URL，而不是信号路径——evlog 会自行追加 `/v1/logs`。
使用默认 OTLP HTTP 端口的自托管收集器地址为 `http://host:4318`。
::

## 工作原理

在底层，`createHyperDXDrain()` 会将你的 HyperDX 设置映射到共享的 [OTLP 适配器](/integrate/adapters/hybrid/otlp)，并调用 `sendBatchToOTLP()`：

- **端点**：OTLP HTTP 基础 URL，默认为 `https://in-otel.hyperdx.io`（evlog 发布到 `{endpoint}/v1/logs`）
- **认证**：`authorization` 标头设置为你的 API 密钥（与 HyperDX 文档中所述的 `otlphttp` 导出器相同）
...
- [OTLP 适配器](/integrate/adapters/hybrid/otlp) - 通过 OpenTelemetry 协议将日志发送到任意 OTLP 后端
- [PostHog 适配器](/integrate/adapters/cloud/posthog) - 通过 OTLP 将日志发送到 PostHog Logs
- [自定义适配器](/extend/custom-drains) - 构建你自己的适配器
- [最佳实践](/reference/best-practices) - 安全性与生产环境提示
