...
每个事件都会作为**JSON 日志行**推送，并归属于一组小型、低基数的标签。在 Loki 中，这一区别非常重要：标签会按基数建立索引并计费，而日志行则在查询时进行搜索。evlog 默认仅为 `service`、`environment` 和 `level` 添加标签，并将其他所有内容（`requestId`、`path`、`user` 以及你的自定义字段）保留在日志行中，可通过 `| json` 进行查询。
...
对于**多租户**部署，请指定租户名称，该名称会作为 `X-Scope-OrgID` 发送：
...
`user` 是 Loki 数据源的**数字实例 ID**。请在 Grafana Cloud 堆栈中的
*Connections → Data sources → Loki* 下查找它。它不是你的账户邮箱。使用错误的值通常会导致 `401`
::
...
绝不要将高基数字段提升为标签。Loki 会为每种唯一的标签组合创建一个流，因此为 `requestId` 或 `userId` 添加标签会创建数百万个流，并导致你的实例性能下降或无法运行
::
...
**`Loki API error: 401`。** 在 Grafana Cloud 中，请检查 `user` 是否为 Loki 数据源的数字**实例 ID**，而不是你的账户邮箱。
...
- [Drain 管道](/extend/drain-pipeline)：批处理、重试和扇出
- [适配器概览](/integrate/adapters/overview)：所有可用的目标站点
