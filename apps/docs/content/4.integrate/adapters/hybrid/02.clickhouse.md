...
默认架构会为你进行筛选和聚合的字段提供**类型化列**，同时将完整的宽事件以 JSON 形式存储在 `data` 中——因此不会丢失任何字段，并且向事件中添加字段永远不需要执行迁移。
...
`ORDER BY (service, environment, timestamp)` 与最常用的筛选方式相匹配。根据你的保留策略调整 `TTL`——如果要永久保留事件，可以完全删除该子句。
::
...
无论采用哪种方式，适配器都完全相同——只有端点和凭据会发生变化。
...
包括 `:8443` 端口。云服务会在空闲时暂停，因此暂停后的首次插入
...
如果想*查看*事件而不是对其进行断言，可以填充沙盒数据并打开已配置的仪表板——该技术栈自带一个 Grafana，其中的 ClickHouse
数据源已经完成连接配置：
...
ClickHouse 内置的 `/play` 页面 `http://localhost:8123/play` 也可用于执行原始
SQL，但它有意保持简洁——仪表板是更好的起点。
...
如果需要在排空操作完成前确认插入已持久化，请设置 `waitForAsyncInsert: true`，但这会增加延迟。设置 `asyncInsert: false` 可进行同步插入；只有在已经通过 `evlog/pipeline` 在客户端进行批处理时，这样做才有意义。
...
键必须与列名匹配——ClickHouse 会拒绝包含未知列的 `JSONEachRow` 插入。
...
**`Missing endpoint`** — 未设置 `CLICKHOUSE_ENDPOINT`，且未传入 `endpoint`。该排空器记录一次 `[evlog/clickhouse] Missing endpoint`，随后不执行任何操作，因此请求永远不会被阻止或失败。
...
**`ClickHouse API error: 400` mentioning unknown column** — 您的表与行结构不匹配。请根据上述 DDL 创建表，或提供与您的列相匹配的 `transform`。

**`Cannot parse input`** — 表已存在，但某列的类型不一致。`status` 是 `Nullable(UInt16)`，因为非 HTTP 事件中不存在该字段。
...
- [采样](/learn/sampling) — 在数据到达 ClickHouse 之前控制数据量
- [丰富器](/use-cases/enrichers) — 为每个事件添加派生字段
- [管道](/learn/pipeline) — 批处理、重试和扇出
- [适配器概览](/integrate/adapters/overview) — 所有可用的目标位置
