---
title: NestJS
description: "`EvlogModule.forRoot()` 会在 NestJS 应用中注册中间件、异常过滤器和 `useLogger()`，并在需要时支持异步配置。"
navigation:
  title: NestJS
...
::prompt
---
description: 在我的 NestJS 应用中设置 evlog
icon: i-simple-icons-nestjs
actions:
  - copy
  - cursor
...
文档：https://www.evlog.dev/integrate/frameworks/nestjs
...
使用 `req.log.fork(label, fn)`（或在同一请求中使用 `useLogger()` 获取的日志记录器）创建子广泛事件。请参阅广泛事件中的[发送后](/learn/wide-events#after-emit-sealing-and-background-work)。
...
在服务器关闭时调用 `drain.flush()`，以确保所有缓冲的事件都已发送。有关所有选项，请参见 [管道文档](/extend/drain-pipeline)。
::
