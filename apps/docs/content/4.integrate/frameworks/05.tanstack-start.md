...
TanStack Start 使用 [Nitro v3](/integrate/frameworks/nitro) 作为其服务器层，因此 evlog 通过 `evlog/nitro/v3` 模块集成。相同的基于插件的钩子系统同样适用。

::callout{icon="i-lucide-info" color="info"}
**TanStack Router vs TanStack Start**：TanStack Router 是一个客户端路由器，不需要服务器端日志记录。本页面介绍的是 **TanStack Start**，即全栈框架。如果你使用的是 SPA 模式下的 TanStack Router，请改看 [客户端日志](/use-cases/client-logging)。
::

::prompt
---
description: 在我的 TanStack Start 应用中设置 evlog
icon: i-custom-tanstack
actions:
  - copy
  - cursor
...
在我的 TanStack Start 应用中设置 evlog。
...
文档：https://www.evlog.dev/integrate/frameworks/tanstack-start
...
**使用 Vite？** TanStack Start 基于 Vite。[`evlog/vite`](/reference/vite-plugin) 插件会从生产构建中移除 `log.debug()` 并注入源代码位置，请将其与 TanStack Start 插件一起添加到你的 `vite.config.ts` 中。
::
...
`log.fork()` 在此集成中不可用。`evlog/nitro/v3` 模块会将记录器附加到请求事件，但不会自行注册 `AsyncLocalStorage` 存储，因此即使启用了 `experimental.asyncContext`，也没有可供子广泛事件运行的作用域（该选项支持的是 Nitro 的 `useRequest()`，而不是 `log.fork()`）。请改为在处理器内部等待后台工作；在广泛事件发出后修改记录器会触发排放后警告。请参阅广泛事件中的[排放后](/learn/wide-events#after-emit-sealing-and-background-work)。
...
查看 [配置参考](/reference/configuration) 了解所有可用选项（`initLogger`、中间件选项、采样、静默模式等）。
...
::callout{icon="i-lucide-info" color="info"}
查看 [适配器](/integrate/adapters/overview) 和 [增强器](/use-cases/enrichers) 文档，了解所有可用的排放适配器和增强器。
::
...
在服务器关闭时调用 `drain.flush()` 以确保所有缓冲事件都已发送。有关所有选项，请参阅 [管道文档](/extend/drain-pipeline)。
::
...
- [广泛事件](/learn/wide-events)：通过上下文分层设计全面事件
