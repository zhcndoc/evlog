...
对于脚本、队列工作者、cron 和 CLI，本页为参考。在 Cloudflare Workers 上，请优先使用 [Cloudflare Workers](/integrate/frameworks/cloudflare-workers)（`createWorkersLogger`）。
::

::prompt
---
description: 在我的 TypeScript 项目中设置 evlog
icon: i-simple-icons-typescript
actions:
  - copy
  - cursor
...
为脚本、工作者或 CLI 工具在我的 TypeScript 项目中设置 evlog。
...
文档：https://www.evlog.dev/integrate/frameworks/standalone
...
**使用 vite-node？** [`evlog/vite` 插件](/reference/vite-plugin) 会用编译时自动初始化替换 `initLogger()` 调用，从生产构建中移除 `log.debug()`，并注入源位置。
::
...
::callout{icon="i-lucide-info" color="info"}
请参阅所有可用排水适配器（Axiom、OTLP、PostHog、Sentry、Better Stack）的 [适配器](/integrate/adapters/overview) 文档。
::

::callout{icon="i-lucide-arrow-right" color="neutral"}
请参阅完整的 [bun-script 示例](https://github.com/evloghq/evlog/tree/main/examples/bun-script)，了解一个完整可运行的脚本。
::

## 下一步

- [Node.js 请求日志记录](/use-cases/structured-logging-nodejs)：将这些基础构件应用于完整的 HTTP 服务器，并跟踪从响应到日志的成功与失败。
...
- [结构化错误](/learn/structured-errors)：使用 `why`、`fix` 和 `link` 字段抛出错误
