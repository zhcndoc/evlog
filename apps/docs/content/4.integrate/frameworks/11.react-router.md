...
::prompt
---
description: 在我的 React Router 应用中设置 evlog
icon: i-custom-reactrouter
actions:
  - copy
  - cursor
...
文档：https://www.evlog.dev/integrate/frameworks/react-router
...
**使用 Vite？** `evlog/vite` [插件](/reference/vite-plugin) 会用编译时自动初始化替换 `initLogger()` 调用，从生产构建中剥离 `log.debug()`，并注入源码位置。
::
...
`loggerContext` 中的日志记录器支持用于创建子广泛事件的 `fork`。请参阅广泛事件中的[发出后](/learn/wide-events#after-emit-sealing-and-background-work)。
...
有关所有可用选项（`initLogger`、中间件选项、采样、静默模式等），请参见 [配置参考](/reference/configuration)。
...
::callout{color="info" icon="i-lucide-info"}
在服务器关闭时调用 `drain.flush()` 以确保发送所有已缓冲事件。有关所有选项，请参见 [管道文档](/extend/drain-pipeline)。
:::
...
- [宽事件](/learn/wide-events)：设计具有上下文分层的完整事件
