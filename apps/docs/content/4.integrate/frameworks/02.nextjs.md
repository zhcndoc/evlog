...
将仅限 Node 的导入（`evlog/fs`、重量级适配器）排除在根目录的 `instrumentation.ts` 之外。使用带有选项对象的 `defineNodeInstrumentation`。evlog 仅在 Node.js 中加载 `createInstrumentation`，不会在你的文件中出现明显的 `import()`。
...
::callout{icon="i-lucide-info" color="info"}
`captureOutput` 仅在 Node.js 运行时（`NEXT_RUNTIME === 'nodejs'`）中激活。它会修补 `process.stdout.write` 和 `process.stderr.write`，以发出结构化的 `log.info` / `log.error` 事件。当 `silent` 为 `false`（默认值）时，捕获的输出会以结构化终端输出的形式显示一次，并且不会重复原始写入。设置 `silent: true` 可在保留排水器传递的同时保留原始直通输出。已知的 Next.js Edge 打包器警告默认会被过滤，因此不会作为 evlog 错误重新发出。
::
...
在 Vercel 上，`withEvlog` 会使用 Next 的 `after()` 注册排水工作；使用管道时，它还会注册 `drain.settled()`：该函数会一直保持活动状态，直到缓冲批次传送完成，或者在重试次数耗尽后被丢弃。`batch.intervalMs` 会限制首次传送尝试开始的时间；重试退避可能会使生命周期超过该时间。如果你的摄取端点响应速度太慢，达到默认的 5 秒请求超时，请提高超时时间（`createAxiomDrain({ timeout: 15000 })`）：实际已经到达目标的超时请求会被重试，而非幂等的摄取 API 会将其计为两次。
::
...
在 `withEvlog` 内部，`useLogger()` 会返回一个带有 **`fork`** 的日志记录器，用于创建子广泛事件。请参阅广泛事件中的[发出后](/learn/wide-events#after-emit-sealing-and-background-work)。
...
在终端中，错误会显示在广泛事件内部：先显示错误块，然后显示请求上下文。颜色和树形连接符会在终端中渲染；下面的示例为便于阅读省略了 ANSI 控制码。
...
使用 `EvlogProvider` 包装根布局，以启用客户端日志记录及其排水器：
