---
title: Nitro
description: 一个模块涵盖 Nitro v3 和 v2。通过 `useLogger(event)` 获取请求日志记录器，并从 Nitro 插件中设置排水器和增强器。
navigation:
  title: Nitro
  icon: i-custom-nitro
---
...
::prompt
---
description: 在我的 Nitro 应用中设置 evlog
icon: i-custom-nitro
actions:
  - copy
  - cursor
...
文档：https://www.evlog.dev/integrate/frameworks/nitro
...
Nitro 使用 **`useLogger(event)`**（事件绑定的作用域），而不是 **`AsyncLocalStorage`**，因此 **`log.fork()`** 目前在这里不可用。对于 AI SDK 流式响应，evlog 会延迟发出广泛事件，直到响应正文完成，这样 `createAILogger(log)` 的元数据就会保留在同一个请求事件中。只有当代码在广泛事件实际发出后调用 `set()` 时，才会出现发出后警告，例如在非流式处理程序或后台工作中。请参阅广泛事件中的[发出后](/learn/wide-events#after-emit-sealing-and-background-work)。
...
请参阅 [配置参考](/reference/configuration)，了解所有可用选项（`enabled`、`pretty`、`silent`、`sampling` 等）。
...
请参阅 [适配器](/integrate/adapters/overview) 和 [增强器](/use-cases/enrichers) 文档，了解所有可用的排水器和增强器列表。
::
