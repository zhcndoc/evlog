---
title: SvelteKit
description: 在 SvelteKit 应用中自动创建宽事件、结构化错误、日志排出适配器和富处理器，并支持尾部采样。
navigation:
  title: SvelteKit
...
对子宽事件使用 `locals.log.fork(label, fn)`。参见 [宽事件 — 发送后](/learn/wide-events#after-emit-sealing-and-background-work)。
