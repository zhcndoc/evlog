---
title: Fastify
description: Fastify 应用中的自动宽事件、结构化错误、排出适配器、增强器和尾部采样。
navigation:
  title: Fastify
...
使用 `request.log.fork(label, fn)` 处理应在响应后发出**单独的**子宽事件的异步工作。参见 [宽事件 — 发送后](/learn/wide-events#after-emit-sealing-and-background-work)。
