---
title: Fastify
description: 该插件会在 `request.log` 上覆盖 Fastify 内置的 pino 日志记录器，并从 onRequest、onResponse 和 onError hook 发出一个宽事件
navigation:
  title: Fastify
...
::prompt
---
description: 在我的 Fastify 应用中设置 evlog
icon: i-simple-icons-fastify
actions:
  - copy
  - cursor
...
文档：https://www.evlog.dev/integrate/frameworks/fastify
...
**使用 Vite？** [`evlog/vite` 插件](/reference/vite-plugin) 会用编译时自动初始化替换 `initLogger()` 调用，从生产构建中剥离 `log.debug()`，并注入源位置。
::
...
使用 `request.log.fork(label, fn)` 执行应在响应之后发出一个**单独**的子宽事件的异步工作。请参阅宽事件中的[发出之后](/learn/wide-events#after-emit-sealing-and-background-work)。
