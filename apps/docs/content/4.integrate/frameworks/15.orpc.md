---
title: oRPC
description: oRPC 应用中的自动宽事件、结构化错误、drain 适配器、enricher 以及尾部采样。
navigation:
  title: oRPC
...
**oRPC v2：** oRPC 维护者已[宣布在 v2 中提供原生 evlog 支持](https://github.com/middleapi/orpc/discussions/1293#discussioncomment-17032656)（已可用于 edge）。在 v2 发布之前，`evlog/orpc` 是 oRPC v1 的集成路径——无论未来 oRPC 如何接入它，它仍然是 evlog 完整流水线（drain、enricher、尾部采样、结构化错误）的入口。
::
...
- 使用 `evlog/orpc` 中的 withEvlog() 包装你的 RPCHandler / OpenAPIHandler
- 在基础 procedure 上添加 os.use(evlog())，以获得带类型的 context.log 和每个 procedure 的 operation
- 在基础 context 上声明 EvlogOrpcContext，以便为 context.log 提供类型
...
::callout{icon="i-lucide-info" color="info"}
oRPC 的错误封装格式是 `{ defined, code, status, message, data }` ——客户端通过 `@orpc/client` 的 `safe()` 将错误反序列化为一个有类型的联合类型。evlog 遵循该协议，因此 `why`/`fix`/`link` 位于 `data` 下，而不是响应根部。编写 API（`createError` / [`defineErrorCatalog`](/learn/structured-errors#error-catalogs)）与 evlog 其余部分完全一致。
:::
