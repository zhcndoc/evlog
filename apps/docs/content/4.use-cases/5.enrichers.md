---
title: 增强器
description: 自动为每个宽事件添加派生上下文——用户代理、地理位置、请求大小和跟踪上下文。来自 evlog/enrichers 的内置增强器，以及如何将它们与你自己的增强器组合使用。
navigation:
  title: 增强器
...
增强器会在广泛事件发出后、到达你的 drain 适配器之前，为其添加派生上下文。可用它们自动从请求头中提取有用信息，而无需让应用代码变得杂乱。

所有内置增强器都从 `evlog/enrichers` 导出。每个增强器都是一个工厂函数，会返回一个 `(ctx: EnrichContext) => void` 回调。若要编写你自己的增强器，请参阅 [自定义增强器](/extend/custom-enrichers)。

::prompt
---
description: 添加所有内置 evlog 增强器
...
Enricher 文档：https://www.evlog.dev/use-cases/enrichers
...
使用 `createDefaultEnrichers()` 一次性组合用户代理、地理位置、请求大小和跟踪上下文：
...
每个增强器都接受 `{ overwrite?: boolean }`（默认值为 `false`），因此用户设置的字段会被保留。
...
**Cloudflare 注：** 只有 `cf-ipcountry` 是标准的 Cloudflare HTTP 标头。其他地理字段（`city`、`region`、`latitude` 等）是 `request.cf` 的属性，而不是作为标头暴露。若要获取完整的 Cloudflare 地理数据，请编写一个读取 `request.cf` 的 [自定义增强器](/extend/custom-enrichers)，或者使用 Workers 中间件将 `cf` 属性复制到自定义标头中。
::
...
一起使用所有内置增强器。增强器列表在各框架中是一致的——只有接线方式不同。
...
- [自定义增强器](/extend/custom-enrichers) - 编写你自己的增强器
- [适配器](/integrate/adapters/overview) - 将增强后的事件发送到外部服务
